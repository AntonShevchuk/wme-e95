...
(function($, WazeApi) {
    ...
    // Update street
    function setupStreet20() {
        let attributes = {
            fwdMaxSpeed: 20,
            revMaxSpeed: 20,
            roadType: types.private,
        };
        setupRoad(attributes);
    }
    function setupStreet50() {
        let attributes = {
            fwdMaxSpeed: 50,
            revMaxSpeed: 50,
    ...
    // Create UI everytime
    function createUI() {
        let group = $('<div class="form-group e95"></div>')
            .append('<label class="control-label">Quick properties</label>')
            .append('<div class="controls"></div>')
            .insertBefore('#segment-edit-general')
        ;

        group.find('.controls')
            .append('<button class="road-20">20</button>')
            .append('<button class="road-50">50</button>')
            .append('<button class="road-90">90</button>')
        ;

        console.log('E95: Interface was loaded');
    }
    ...
    function init() {
            ...
            mutations.forEach(function(mutation) {
                for (let i = 0, total = mutation.addedNodes.length; i < total; i++) {
                    ...
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        if (node.querySelector('div.controls.speed-limit') &&
        ...
        // Handlers
        $('#edit-panel')
            .on('click', '.road-20', setupStreet20)
            .on('click', '.road-50', setupStreet50)
            .on('click', '.road-90', setupStreet90)
        ;
    }
